...
export default function App(){
  ...
  async function addRes(r){try{const id=await monday.createResource(r);setData(p=>({...p,resources:[...p.resources,{title:r.title,description:r.description,type:r.type,category:r.category,id,createdAt:TODAY,hasFile:!!r.file,fileName:r.file?.name||""}]}));}catch(e){console.error("addRes failed",e);}}
  async function viewFile(id){try{const url=await monday.getResourceFileUrl(id);if(url)window.open(url,"_blank");else alert("Couldn't find a file for this resource.");}catch(e){console.error("viewFile failed",e);alert("Couldn't open file.");}}
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,color:C.text,minHeight:"100vh",display:"flex"}}>
      ...
      <main style={{flex:1,padding:"24px 32px",overflowY:"auto",maxHeight:"100vh"}}>
        ...
        {page==="resources"&&<ResourcesPage data={data} filter={rF} setFilter={setRF} setModal={setModal} viewFile={viewFile}/>}
...
function ResourcesPage({data,filter,setFilter,setModal,viewFile}){
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>{fil.map(r=>{const lk=data.tasks.filter(t=>t.resourceId===r.id);return<div key={r.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:16,transition:"border-color 0.2s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=C.bHover} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
      ...
      {r.hasFile&&<button onClick={()=>viewFile(r.id)} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,borderRadius:6,padding:"5px 10px",color:C.primary,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",marginBottom:8}}>{I.doc} View File{r.fileName?` · ${r.fileName}`:""}</button>}
...
function AddTaskModal({data,onClose,onAdd}){
  const[f,setF]=useState({title:"",description:"",category:"Sales",priority:"medium",dueDate:shD(TODAY,14),assignedTo:[],resourceId:""});const s=(k,v)=>setF(p=>({...p,[k]:v}));const tgl=id=>s("assignedTo",f.assignedTo.includes(id)?f.assignedTo.filter(x=>x!==id):[...f.assignedTo,id]);const ok=f.title&&f.assignedTo.length;
...
function AddResourceModal({onClose,onAdd}){
  const[f,setF]=useState({title:"",description:"",type:"pdf",category:"Sales",file:null});const[busy,setBusy]=useState(false);const s=(k,v)=>setF(p=>({...p,[k]:v}));
  return<Modal title="Add Resource" onClose={onClose} width={420}><div style={{marginBottom:12}}><label style={sL}>Title *</label><input value={f.title} onChange={e=>s("title",e.target.value)} style={sI} autoFocus/></div><div style={{marginBottom:12}}><label style={sL}>Description</label><textarea value={f.description} onChange={e=>s("description",e.target.value)} rows={2} style={{...sI,resize:"vertical"}}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}><div><label style={sL}>Type</label><select value={f.type} onChange={e=>s("type",e.target.value)} style={sS}>{["pdf","video","doc"].map(t=><option key={t}>{t.toUpperCase()}</option>)}</select></div><div><label style={sL}>Category</label><select value={f.category} onChange={e=>s("category",e.target.value)} style={sS}>{["Sales","Product Knowledge","Operations","Partnerships","Compliance"].map(c=><option key={c}>{c}</option>)}</select></div></div><div style={{marginBottom:16}}><label style={sL}>File (optional)</label><input type="file" onChange={e=>s("file",e.target.files[0]||null)} style={{...sI,padding:"7px 10px"}}/>{f.file&&<p style={{margin:"6px 0 0",fontSize:11,color:C.muted}}>{f.file.name}</p>}</div><button disabled={busy} onClick={async()=>{if(f.title){setBusy(true);await onAdd(f);setBusy(false);onClose();}}} style={bP(!!f.title)}>{busy?"Uploading…":"Add Resource"}</button></Modal>;
}
